...
class MyScore extends Component {
  ...
  actClick=(e)=>{
    router.push("/activity/actmark?id="+e);
  }
          ...
          <div className="container">
            <div className={styles.chooseCon}>
              <ul className={styles.btnList}>
                ...
                <li><Link to='/activity/myscore' style={{color:"#6fc400"}}>我参与的评分</Link></li>
              </ul>
            </div>
                  ...
                  this.state.listData.map((item,index)=>{
                    return (
                      ...
                      <div className={styles.timeCon}>
                        {/* <i style={{backgroundImage: `url(${require('@/assets/edit.png')})`}} onClick={()=>{this.actClick(item.id)}}> </i> */}
                        {/* <Link to="/activity/actmark?actId="></Link> */}
                        <div className={styles.time}>{util.dateDeal(item.act_star_date)} 至 {util.dateDeal(item.act_end_date)}</div>
                      </div>
                      <p className={styles.word} onClick={()=>this.actClick(item.id)}>{item.name}</p>
                      </li>
                    )
                  })
                }
