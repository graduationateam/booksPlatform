...
import { Modal, Input, Row, Col, Button, Affix, Rate } from 'antd';
...
class SellDetail extends Component {
  ...
  getOther = (id,e)=> {
    const { dispatch } = this.props;
    router.push("/resource/selldetail?id="+id);
    this.getData(id);
  }
  seeReport = ()=> {
    this.setState({
  ...
  render() {
    const { reportVisible, theData, relevantList, hasGood, goodNum, reportIconVisible, 
      starVisiable, collectVisible, starValue, buildVisible, isDeleteCollect } = this.state;
    return (
      <div>
        <Zheader addr={2}/>
        <div className={styles.moreCon}>
          <div className="container">
            <div className={styles.relatedCon} style={{marginRight:'30px'}}>
              {utilsView.renderLineTitle("相关书籍")}
            ...
            <div className={styles.articleCon}>
              ...
              <div className={styles.article}>
                  <div className={styles.title}>{theData.userName}出售的二手书 > {theData.name}</div>
                  ...
                  <li>
                    ...
                    <div className={styles.textInfo}>
                        ...
                        <div className={styles.textCon}>
                          ...
                          <div>￥{theData.price} <span style={{textDecoration:'line-through'}}>￥{theData.originalPrice}</span></div>
                ...
                <Affix offsetTop={0} offsetBottom={0}>
                  <div className={styles.handleCon} onMouseEnter={this.seeReport} onMouseLeave={this.hiddenReport}>
                    <div className={styles.handleBtn}>
                      {
                        theData.is_collection?
                        <span style={{color:'#ed3624'}}><i className={styles.iconHasCollect}></i>取消收藏 {theData.collect_qty}</span>
                        :<span><i className={styles.iconCollect}></i><span className={styles.collectText}>收藏 {theData.collect_qty}</span></span>
                      }
                      <span onClick={this.showBuilding}><i className={styles.iconShare}></i><span>分享</span></span>
                      { reportIconVisible?<span onClick={this.showBuilding}><i className={styles.iconReport}></i><span>举报</span></span>:'' }
                    </div>
                    <div className={styles.addBtn}>
                      <span className={styles.freeDown}><i></i>加入购物车</span>
                      <span className={styles.addToBasket} onClick={this.showBuilding}><i></i>立即购买</span>
                    </div>
                  </div>
                </Affix>
              </div>
